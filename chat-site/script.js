...
let adminUsers = ['themethekeythesee22@example.com'];
let userConversations = {}; // { dmId: true }
...
// Listen to user's DM list in Firestore (cross-platform/cross-tab)
...
// Render DM list, always fetch latest username for each DM
function renderSavedConversations() {
    ...
    Object.keys(userConversations).forEach(dmId => {
        ...
        const otherUserId = ids.find(id => id !== currentUser.uid);
        ...
        db.collection('users').doc(otherUserId).get().then(userDoc => {
            ...
            addDMToList(dmId, username, currentChannel === `dm_${dmId}`);
        });
    });
}

// Save DM id to Firestore (no username stored, always up-to-date)
function saveConversation(dmId) {
    if (!currentUser) return;
    userConversations[dmId] = true;
...
// Add DM to the list (with username, clickable, active state)
function addDMToList(dmId, username, makeActive) {
    const { dmList } = getDOM();
    if (!dmList) return;
    const existingDm = dmList.querySelector(`[data-dm-id="${dmId}"]`);
    if (existingDm) {
        existingDm.textContent = `@${username}`;
        if (makeActive) existingDm.classList.add('active');
        else existingDm.classList.remove('active');
        return;
    }
